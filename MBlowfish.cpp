...
void MBlowfishImpl::_InitTable(const char *passwd, size_t len)
{
    ...
    j = 0;
    for (i = 0; i < bf_pa_size; i++)
    {
        work.bytes.zero = passwd[(j++) % len];
        work.bytes.one = passwd[(j++) % len];
        work.bytes.two = passwd[(j++) % len];
        work.bytes.three = passwd[(j++) % len];
        m_pa[i] ^= work.dword;
    }

    null0.dword = null1.dword = 0;
...
void MBlowfishImpl::_Enc(BF_Dword *x1, BF_Dword *x2)
{
    ...
    w1.dword ^= m_pa[0];
    w2.dword ^= F(w1) ^ m_pa[1];       w1.dword ^= F(w2) ^ m_pa[2];
    w2.dword ^= F(w1) ^ m_pa[3];       w1.dword ^= F(w2) ^ m_pa[4];
    w2.dword ^= F(w1) ^ m_pa[5];       w1.dword ^= F(w2) ^ m_pa[6];
    w2.dword ^= F(w1) ^ m_pa[7];       w1.dword ^= F(w2) ^ m_pa[8];
    w2.dword ^= F(w1) ^ m_pa[9];       w1.dword ^= F(w2) ^ m_pa[10];
    w2.dword ^= F(w1) ^ m_pa[11];      w1.dword ^= F(w2) ^ m_pa[12];
    w2.dword ^= F(w1) ^ m_pa[13];      w1.dword ^= F(w2) ^ m_pa[14];
    w2.dword ^= F(w1) ^ m_pa[15];      w1.dword ^= F(w2) ^ m_pa[16];
    w2.dword ^= m_pa[17];
...
void MBlowfishImpl::_Dec(BF_Dword *x1, BF_Dword *x2)
{
    ...
    w1.dword ^= m_pa[17];
    w2.dword ^= F(w1) ^ m_pa[16];      w1.dword ^= F(w2) ^ m_pa[15];
    w2.dword ^= F(w1) ^ m_pa[14];      w1.dword ^= F(w2) ^ m_pa[13];
    w2.dword ^= F(w1) ^ m_pa[12];      w1.dword ^= F(w2) ^ m_pa[11];
    w2.dword ^= F(w1) ^ m_pa[10];      w1.dword ^= F(w2) ^ m_pa[9];
    w2.dword ^= F(w1) ^ m_pa[8];       w1.dword ^= F(w2) ^ m_pa[7];
    w2.dword ^= F(w1) ^ m_pa[6];       w1.dword ^= F(w2) ^ m_pa[5];
    w2.dword ^= F(w1) ^ m_pa[4];       w1.dword ^= F(w2) ^ m_pa[3];
    w2.dword ^= F(w1) ^ m_pa[2];       w1.dword ^= F(w2) ^ m_pa[1];
    w2.dword ^= m_pa[0];
...
void MBlowfish::Reset()
{
    ...
    sb_init[bf_s_boxes][bf_num_entries] =
    {
    ...
    assert(sizeof(m_pimpl->m_pa) == sizeof(pa_init));
    assert(sizeof(m_pimpl->m_sb) == sizeof(sb_init));
    memcpy(m_pimpl->m_pa, pa_init, sizeof(m_pimpl->m_pa));
...
uint8_t *MBlowfish::EncryptWithLength(const void *ptr, uint32_t& length)
{
    ...
    uint32_t new_length = 8 + fixed_length;
    assert(new_length % 8 == 0);
    assert(new_length >= length);

    auto encrypted = new uint8_t[new_length];
    memset(encrypted, 0, new_length);
    memcpy(encrypted, &length, sizeof(uint32_t));
    memcpy(encrypted + sizeof(uint32_t), &fixed_length, sizeof(uint32_t));
...
uint8_t *MBlowfish::DecryptWithLength(const void *ptr, uint32_t& length)
{
    ...
    uint32_t new_length, fixed_length;
    memcpy(&new_length, decrypted, sizeof(uint32_t));
    memcpy(&fixed_length, decrypted + sizeof(uint32_t), sizeof(uint32_t));
    ...
    memmove(decrypted, content, new_length);
    decrypted[new_length] = 0;
...
int main(void)
{
    auto orig = "This is the original text. This is the original text.";
    auto passwd = "This is a password.";

    printf("Password: %s\n", passwd);

    uint32_t len = uint32_t(std::strlen(orig));
    printf("Original: %s\n", orig);
    printf("Length: %d\n", int(len));
